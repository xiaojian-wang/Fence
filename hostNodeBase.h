...
class hostNodeBase : public cSimpleModule {
    protected:
        ...
        unordered_map<int, PaymentChannel> nodeToPaymentChannel; //Note: all the channel with node <int> Question: one node can have multiple channel, which one this value??
        ...
        unordered_map<int, unordered_map<int, PathInfo>> nodeToShortestPathsMap = {};  // Note: destination, pathindex, pathinfo

        //Note: centralized change, this set also should be global, not belong to a node
        // //OnlineScheme: store all the tx already go through
        // set<int> transactionIdSet = {}; // Note: FIXMEd: what type of the transaction id <txid:int,success/fail>
        // // TODO: How to update this set?
        ...
        // accordingly and how much you've sent to each receiver
    ...
    public:
        ...
        virtual void handleTransactionMessageSpecialized(routerMsg *msg); //Note: overwrite this function for each scheme
        ...
        //Note: stat the network balance state
        virtual void statNetworkChannelBalance(string topologyFile, string balanceRecordFile);
};
