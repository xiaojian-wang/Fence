...
int routerNodeBase::myIndex(){
   return getIndex() + _numHostNodes; //Note: the index of router except host
}
...
void routerNodeBase::checkQueuedTransUnits(vector<tuple<int, double, routerMsg*,  Id, simtime_t>> 
    ...
    cout << "routerNodeBase::checkQueuedTransUnit" << endl;
...
void routerNodeBase::deleteTransaction(routerMsg* ttmsg) {
    cout << "routerNodeBase::deleteTransaction" << endl;
...
void routerNodeBase::initialize()
{
    ...
    if (_loggingEnabled) {
    cout << "***" << endl;
    cout << "Start to initialize the router!" << endl;
    }
    ...
    if (_timeoutEnabled) {
        cout << "generate clear state message since the time out at " << simTime() + _clearRate  << endl;
 ...
 */
//Note: before pop, the message still 0, after pop, it is 1
//Note: why we need pop?

bool routerNodeBase:: processTransUnits(int neighbor, 
        vector<tuple<int, double , routerMsg *, Id, simtime_t>>& q) {
    // cout << "-=-= ack message input?? test6-=-=" << endl;
    // cout << "message type: " << ttmsg->getMessageType() << endl;
    int successful = true;
    while ((int)q.size() > 0 && successful == 1) {
        cout << "-=-= ack message input?? test7-=-=" << endl;
        cout << "message type: " << get<2>(q.back())->getMessageType() << endl;
...
routerMsg *routerNodeBase::generateAckMessage(routerMsg* ttmsg, bool isSuccess) {
    ...
    // cout << "in generateACK: new ack message name: " << msgname << endl;
    ...
    reverse(route.begin(), route.end()); // in the ACK, the route is reversed
    msg->setRoute(route);
    ...
    // cout << "after delete transMsg and ttmsg, since we generate new aMsg message" << endl;
    return msg;
}
...
void routerNodeBase::forwardMessage(routerMsg* msg){
    if (_loggingEnabled) {
    cout << "forward Message at router!!" << endl;
    cout << "current hop is: " << msg->getHopCount() << endl;
    }
...
int routerNodeBase::forwardTransactionMessage(routerMsg *msg, int neighborIdx, simtime_t arrivalTime) {
    cout << "-=-= ack message input?? test5-=-=" << endl;
    cout << "message type: " << msg->getMessageType() << endl;
    transactionMsg *transMsg = check_and_cast<transactionMsg *>(msg->getEncapsulatedPacket());
    cout << "-=-= ack message input?? test6-=-=" << endl;
    ...
    if (neighbor->balance <= 0 || transMsg->getAmount() > neighbor->balance){
        ...
        if (_loggingEnabled) cout << "router forwardTransactionMsg send at: " << simTime() << endl;
        send(msg, nodeToPaymentChannel[nextDest].gate);
        cout << "after send out this message" << endl;
        return 1;
    } 
}
...
void routerNodeBase::handleMessage(cMessage *msg){
    // cout << "segementation fault in routerNodeBase?" << endl;
    ...
    switch (ttmsg->getMessageType()) {
        case ACK_MSG:
            if (_loggingEnabled) 
                cout << '\n' << "[ROUTER "<< myIndex() <<": RECEIVED ACK MSG] " << msg->getName() << endl;
            if (_timeoutEnabled)
        ...
        case UPDATE_MSG:
            ...
            if (_loggingEnabled) cout<< "[AFTER HANDLING:ROUTER RECEIVED UPDATE MSG ====================================================================]  "<< endl;
            break;
...
void routerNodeBase::handleTransactionMessage(routerMsg* ttmsg) {
    cout << "just in handletransactionmessagge message type: " << ttmsg->getMessageType() << endl;
    transactionMsg *transMsg = check_and_cast<transactionMsg *>(ttmsg->getEncapsulatedPacket());
    
    int hopcount = ttmsg->getHopCount();
    vector<tuple<int, double , routerMsg *, Id, simtime_t>> *q; // Note: the queue? in the tuple, the get 2 is message, third element in message
    ...
    if (_hasQueueCapacity && _queueCapacity == 0) {
        ...
        push_heap((*q).begin(), (*q).end(), _schedulingAlgorithm); //Note: insert one element to the heap
        // cout << "-=-= ack message input?? test4-=-=" << endl;
        cout << "message type: " << ttmsg->getMessageType() << endl;
        cout << "message type: " << get<2>(q->back())->getMessageType() << endl;
        processTransUnits(nextNode, *q);
        
    }
    cout << "-=-= ack message input?? end-=-=" << endl;
}
...
bool routerNodeBase::handleTransactionMessageTimeOut(routerMsg* ttmsg) {
    cout << "routerNodeBase::handleTransactionMessageTimeOut" << endl;
    transactionMsg *transMsg = check_and_cast<transactionMsg *>(ttmsg->getEncapsulatedPacket());
    int transactionId = transMsg->getTransactionId();
    auto iter = canceledTransactions.find(make_tuple(transactionId, 0, 0, 0, 0));
...
void routerNodeBase::handleTimeOutMessage(routerMsg* ttmsg){
    cout << "routerNodeBase::handleTimeOutMessage" << endl;
...
void routerNodeBase::handleAckMessage(routerMsg* ttmsg){
    cout << "routerNodeBase::handleAckMessage" << endl;
    ...
    if (aMsg->isSuccess() == false){
        ...
        // routerMsg* uMsg =  generateUpdateMessage(aMsg->getTransactionId(), prevNode, 
                // aMsg->getAmount(), aMsg->getHtlcIndex() ); // Note: we do not need update message
...
void routerNodeBase::handleAckMessageTimeOut(routerMsg* ttmsg){
    cout << "routerNodeBase::handleAckMessageTimeOut" << endl;
    ackMsg *aMsg = check_and_cast<ackMsg *>(ttmsg->getEncapsulatedPacket());
    int transactionId = aMsg->getTransactionId();
    auto iter = canceledTransactions.find(make_tuple(transactionId, 0, 0, 0, 0));
    if (iter != canceledTransactions.end()) {
...
void routerNodeBase::handleUpdateMessage(routerMsg* msg) {
    // cout << "routerNodeBase::handleUpdateMessage" << endl;
    vector<tuple<int, double , routerMsg *, Id, simtime_t>> *q;
